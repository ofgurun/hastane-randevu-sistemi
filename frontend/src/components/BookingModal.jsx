...
import { X, Loader2, CheckCircle2, AlertCircle, CalendarDays } from "lucide-react";
...
export default function BookingModal({ doctor, onClose }) {
  ...
  const onDateChange = async (e) => {
    ...
    try {
      ...
      setSlotsError("Uygun saatler yüklenemedi.");
    } finally {
  ...
  const book = async (slot) => {
    ...
    try {
      ...
      setSuccess(`Randevunuz oluşturuldu: ${date} — saat ${slot}`);
      setSlots([]); // slot listesini temizle
    } catch (err) {
        ...
        <div className="space-y-4 px-6 py-5">
          {success ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <div className="w-full rounded-lg border border-green-200 bg-green-50 px-4 py-3 font-medium text-green-700">
                {success}
              </div>
              <button
                onClick={onClose}
                className="mt-1 rounded-lg bg-blue-600 px-5 py-2 font-semibold text-white transition hover:bg-blue-700"
              >
                Tamam
              </button>
            </div>
